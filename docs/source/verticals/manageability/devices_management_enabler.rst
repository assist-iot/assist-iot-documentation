...
*****************************
The enabler displays a **topology graph of the K8 clusters** registered with the Smart Orchestrator, including all the K8 nodes that make up each cluster.
...
.. note:: 
  The Kubernetes clusters topology feature is stil under develoment, so only the K8s clusters displayed are real information. The K8 nodes and the list of deployed enablers 
  in a cluster are shown as samples to show the future apperance of this feature. Furthermore, in future versions the user will be allowed to click on a node and deploy 
  an enabler in it.
